...
void increase()
{
	static int score = 0; // 변수를 외부에서 사용하지 못하도록 보호하기 위하여 함수 안에서 선언 -> 캡슐화
...
#pragma region CODE 영역
	...
	// 프로세스가 실행할 코드와 매크로 상수가 기계 형태로 저장되는 공간
...
#pragma region BSS 영역
	...
	// 초기화되지 않은 전역 변수가 저장되는 공간

#pragma endregion

#pragma region STACK 영역

	// 함수의 호출과 관계되는 지역 변수와 매개 변수가 저장되는 공간
...
#pragma region 지역 변수
	...
	// { } 벗어나는 시점에서 메모리가 해제되는 특징을 가지고 있는 변수
...
#pragma region 전역 변수
	...
	// 함수 외부에서도 접근이 가능하며,
	// 프로그램이 실행될 때 메모리에서 생성되고,
	// 프로그램이 종료되어야만 메모리에서 사라지는 특징을 가진 변수
...
#pragma region 정적 변수
	...
	// 프로그램이 실행될 때 메모리에 생성되고 프로그램이 종료 되어야만
	// 메모리에서 해제되는 특징을 가지고 있는 변수
...
#pragma region 동적 배열(calloc)
	...
	// ptr = calloc(5, sizeof(int)); // 주소값을 넣을 땐 역참조 X
